...
SELECT pizza_id FROM order_details GROUP BY pizza_id HAVING SUM(quantity) >=10 ORDER BY SUM(quantity) DESC;

-- 6. `order_details` 테이블에서 피자별 총 수익을 `total_revenue` 라는 이름으로 구하세요. (수익 = `quantity * price`)
SELECT od.pizza_id, SUM(od.quantity * p.price) AS total_revenue FROM order_details od JOIN pizzas p ON  p.id = od.pizza_id GROUP BY od.pizza_id ;

-- 7. 날짜별로 피자 주문 건수(`order_count`)와 총 주문 수량(`total_quantity`)을 구하세요.
SELECT o.date, COUNT(o.id) AS order_count, SUM (od.quantity) AS total_quantity FROM orders o JOIN order_details od ON o.id = od.order_id GROUP BY o.date;
